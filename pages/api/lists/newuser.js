    ...
    case "GET":
      try {
        ...
        if (session) {
          ...
          if (hasSavedLists) {
            id = await JSON.parse(JSON.stringify(results2[0].listid));
            uid = await JSON.parse(
              JSON.stringify(results2[0].uid ? results2[0].uid : "")
            );
          }
        }
        ...
        newUser = new Date(user?.createdAt).getTime() > new Date() - 30 * 1000;
